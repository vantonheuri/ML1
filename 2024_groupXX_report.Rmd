...
title: |
  ...
  <span class="real-estate-pricing">Real Estate Pricing</span>
...
header {
  background: linear-gradient(to right,#7A6CBF, #675A9E);
  padding: 20px;
  text-align: center;
  color: white;
}
...
In order to predict rental prices for the real estate company Zug Estates, we used different models, optimised them and fitted them with different features. Specifically, we used a linear model, a generalised linear model with Poisson and binomial families, a generalised additive model, a neural network and a support vector machine. These different approaches allowed us to develop a robust model tailored to the unique characteristics of the property market.
...
# Plot
plot(rel_data$Nr_rooms, rel_data$Price_Gross, pch=19, main="Multiple Linear Models", xlab="Number of Rooms", ylab="Price_Gross", cex=0.5)
...
```{r GAM plot 1, echo=FALSE, warning = FALSE}
...
The plots show that larger properties generally command higher rents, but this effect is less significant when considering the number of rooms.

- **Log-transformed property size** has a strong positive effect on rental prices.

- **Property size adjusted by the number of rooms** has a minimal effect on rental prices.
...
```{r GAM Plot 2, echo=FALSE, warning = FALSE}
...
kbl(conf_matrix_df_nn, align="c") %>%
  kable_classic(full_width=F, html_font="Segoe UI") %>%
  kable_styling(latex_options = c("striped", "hold_position"), position="center", font_size=10) %>%
  column_spec(2:ncol(conf_matrix_df_nn), width="2cm") %>%
  row_spec(0, background = "#9C8AE6", color="white", bold=TRUE)
```
...
kbl(conf_matrix_df_svm, align="c") %>%
  kable_classic(full_width=F, html_font="Segoe UI") %>%
  kable_styling(latex_options = c("striped", "hold_position"), position="center", font_size=10) %>%
  column_spec(2:ncol(conf_matrix_df_svm), width="2cm") %>%
  row_spec(0, background = "#9C8AE6", color="white", bold=TRUE)
...
The dataset is somewhat limited due to the niche nature of the listings market, which is not massive by any means. Moreover, our focus has been concentrated on a few select cantons, further restricting the breadth of data available. Additionally, the data exhibited imbalances in several respects, such as the proportion of houses versus apartments, and disparities among different cantons. Despite these limitations, we have managed to overcome these challenges to a considerable extent, ensuring that our analyses remain robust and insightful. 

## Enhanced Validation of Models 

Due to time constraints and the project's scope, we couldn't conduct extensive validation and testing for all models. Implementing more techniques during the training phase would be essential to improve the overall quality and performance of the developed models. Additionally, testing interactions was limited in this project. 

##  Model Complexity vs. Performance 

In our project, we tested many different machine learning models to find the best ones. While more complex models can sometimes perform better, they can also overfit the data, meaning they work well on training data but not on new, unseen data. 

To avoid overfitting, we decided to use models that are better at generalizing to new data in the long term. These models might not capture every tiny detail in the training data, but they are more reliable and work better in real-world situations. 

By carefully selecting models that balance complexity and performance, we ensure that our results are accurate and useful over time. This approach helps us achieve strong, dependable outcomes for our analyses. 
...
This project has provided a comprehensive analysis of the real estate market in Central Switzerland, specifically tailored to the needs of Zug Estates. By leveraging various machine learning models, we have developed robust methods for predicting rental prices, understanding market trends, and identifying high-value properties. Our exploration included linear models, Generalized Linear Models (GLM), Generalized Additive Models (GAM), neural networks, and support vector machines (SVM), each offering unique insights and validating our predictions. Despite the limitations in our dataset and time constraints, we have successfully demonstrated the potential for advanced data-driven strategies to enhance real estate investment decisions and improve overall market understanding. 

Our findings underscore the importance of balancing model complexity with performance to avoid overfitting and ensure generalizability. We emphasized models that generalize well in the long term, ensuring reliable predictions across different market conditions. 

This project represents a significant step towards more informed, data-driven decision-making in the real estate sector, aligning with Zug Estates' strategic goals and expanding their competitive edge in the market while it also aligns with our desire to keep learning. 

  
# Continuation of the project with Zug Estates. 

After completing our work, we considered that it could be interesting to share our project with Zug Estates due to our passion for the subject and its relevance to them. We believe that our findings and efforts align well with the interests of Zug Estates, making it a valuable collaboration opportunity. 

Therefore, we have written to the Zug Estates Team to explore the possibility of continuing this endeavor together. 

<br> ![](images/email.jpg){width="14in"} <br>
...
- *[1] Homegate. (n.d.). Retrieved June 4, 2024, from https://www.homegate.ch/en*

- *[2] Swissinfo. (2023, April 25). Residential property prices continue to rise in Switzerland. Retrieved June 4, 2024, from https://www.swissinfo.ch/eng/business/residential-property-prices-continue-to-rise-in-switzerland/49166334*

- *[3] OpenAI. (n.d.). Retrieved June 4, 2024, from https://openai.com/es-ES/*

- *[4] Google. (n.d.). Gemini. Retrieved June 4, 2024, from https://gemini.google.com/app*
