...
model_comparison <- data.frame(
  ...
  `R-squared (Adj)` = c(0.389, 0.413, 0.431, 0.388, 0.449),
  `Deviance Explained` = c("39.5%", "41.9%", "43.9%", "39.6%", "45.7%"),
  `GCV Score` = c(754590, 726970, 685020, 759320, 683690)
)

# Create the table with styling
kbl(model_comparison, caption = "Model Comparison Summary", align = "l") %>%
...
In this analysis, we aimed to predict property prices using a Generalised Additive Model (GAM) based on several factors including property size (Size_m2), days difference (Days_Difference), number of rooms (Nr_rooms), GDP per capita (GDP_per), population, area (Area_km2) and density. We first performed a 10-fold cross-validation to assess the performance of the model and found the average RMSE to be an appropriate measure of model accuracy.

Following cross-validation, we fitted the GAM model to the full dataset to more accurately capture the underlying patterns. To understand the effect of property size on predicted prices, we generated a new dataset with Size_m2 values ranging from 50 to 500 m² and predicted prices using the fitted model. The other predictors were randomly sampled from the original dataset to maintain variability.
