...
This chapter delves into an in-depth exploration of the dataset to gain a comprehensive understanding of the underlying data, with a particular focus on the key variable of interest: __prices__. As highlighted in the introduction, our primary objective in this section is to predict the rental prices of apartment and houses.

Before embarking on our data analysis journey, it was essential to prepare our data for the task ahead.

The first step was to import our two datasets, named __"df_kanton"__ (containing information on the Swiss cantons) and __"df_homegate"__ (containing information on the features of properties, such as prices, size, number of rooms, etc). In order to work with one dataset, we merged the two datasets based on the __"Canton"__ column into a new data frame.

However, not all data points fit our intended use case. As mentioned in the introduction, our goal is to predict rental prices for houses and apartments. Therefore, we had to filter out other types of property that didn't fit this purpose.

We then determined that analysing the entire Swiss market was irrelevant for our purposes. We therefore narrowed our focus to Central Switzerland, where we have the greatest market relevance and the property types are more homogeneous. To this end, we applied filters to the data frame, including only records with specific cantonss __Aargau (AG)__, __Luzern (LU)__, __Zurich (ZH)__ and __Zug (ZG)__.

Once the initial data had been collected, we conducted a preliminary cleaning process, which involved carrying out a few visualizations to identify any interesting features that might warrant further attention. This process enabled us to identify which cantons required our attention.

In this second cleaning instance, we also needed to perform the following step:

__1. Handle missing values.__ The process identifies missing values and removes rows with missing values in specific columns, namely "Price_Gross" and "Size_m2".

__2. Feature Engineering.__ A new column, named "Days_Difference", is created and populated with the time difference between two date columns.

__3. One-Hot Encoding.__ One-hot encoding is applied to categorical variables.

__4. Label Encoding.__ This process involves the creation of numerical representations for categorical variables through the use of label encoding.

In summary, these codes prepare the data for further analysis or modelling by cleaning it, handling missing values, and encoding categorical variables, which are common steps in data pre-processing for machine learning tasks.

Here we can observe the different column names of our final dataset.
...
ggplot(properties_per_canton, aes(x = reorder(Canton, -Count), y = Count, fill = color)) +
  ...
  geom_text(aes(label = Count), vjust = -0.5, color = "black", size = 2.2) + 
  scale_fill_identity() + 
...
This visualization indicates significant variability in average rental prices across cantons. The highest average rental prices are observed in Ticino (TI), far exceeding other cantons like Zurich (ZH) and Vaud (VD). This may suggest a premium on rental prices in TI or a presence of high-end properties skewing the average.
...
library(dplyr)
library(gridExtra)
# Calculate the order of cantons based on average rental price
canton_order <- df_total %>%
  group_by(Canton) %>%
  summarise(Average_Price_Gross = mean(Price_Gross, na.rm = TRUE)) %>%
  arrange(desc(Average_Price_Gross)) %>%
...
# Main plot with all cantons ordered by average rental price
p1 <- ggplot(df_total, aes(x = Canton, y = Price_Gross)) +
  stat_summary(fun = "mean", geom = "bar", fill = "skyblue", color = "black") +
  labs(title = "Average Rental Price per Canton", x = "Canton", y = "Average Rental Price (CHF)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

# Define highlighted cantons
highlight_cantons <- c("ZG", "ZU", "AG", "LU")

# Filter data for highlighted cantons and recalculate averages
...
# Adjust the factor levels based on the order in the highlighted cantons
highlighted_data$Canton <- factor(highlighted_data$Canton, levels = highlight_cantons)

# Plot for highlighted cantons
p2 <- ggplot(highlighted_data, aes(x = Canton, y = Average_Price_Gross)) +
  geom_bar(stat = "identity", fill = "coral", color = "black") +
  labs(title = "Highlighted Cantons: Average Rental Price", x = "Canton", y = "Average Rental Price (CHF)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
The box plots compare the gross rental prices between private and professional customer segments. Prices for private customers are generally lower and less varied than those for professionals, indicating a potential market segmentation by rental price.
...
A scatter plot with a trend line demonstrates a positive correlation between property size and rental price, especially noticeable in the Zurich (ZH) canton. This correlation suggests that larger properties tend to command higher rents. We also clustered the points per Canton.
...
*(Canton 1: Argau, Canton 2: Luzern, Canton 3: Zürich)*
...
In the illustration above, we depict the relationship between the number of rooms in a property and the gross price of the same. We can observe an upward curve, which leads to believe that a higher number of rooms leads to a higher price, which would make sense. This information must also be taken into consideration together with other variables, like the property's area in meters squared, average size of rooms, location, etc.
...
As we can see, comparing the two confusion matrices, the Neural Network has better accuracy than the SVM. Though at first glance it may seem like the better option, there are several advantages and disadvantages between the two models.

An SVM is particularly effective when working with higher-dimensional data, and has better memory efficiency. It is also more robust to overfitting, but are less efficient with larger datasets because of the training complexity.
